...
//          GET /api/admin  (routed here as ?action=admin)
...
async function handleAdmin(res) {
  ...
  (userRoles || []).forEach(u => { const r = u.role || 'patient'; roleBreakdown[r] = (roleBreakdown[r] || 0) + 1; });

  return res.status(200).json({
    success: true,
    data: {
      stats: { totalUsers: totalUsers || 0, totalBookings: totalBookings || 0, totalOrders: totalOrders || 0, totalProducts: totalProducts || 0, totalRevenue },
      roleBreakdown, statusBreakdown,
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.query.action === 'admin') return await handleAdmin(res);

    // Patient/doctor dashboard
    const { data: users, error: userError } = await supabase.from('users').select('*').eq('id', userId).limit(1);
    if (userError || !users || users.length === 0)
      return res.status(404).json({ success: false, message: 'User not found' });

    const user = users[0];
    const { data: results } = await supabase.from('chatbot_results').select('*')
      .eq('user_id', userId).order('created_at', { ascending: false }).limit(1);
    const latestResult = results && results.length > 0 ? results[0] : null;

    const { data: history } = await supabase.from('chatbot_results')
