...
//          GET /api/doctors          (routed here as ?action=doctors)
...
module.exports = async function handler(req, res) {
  ...
  // ── Doctors list (no auth needed) ─────────────────────────────
  if (req.query.action === 'doctors') {
    if (req.method !== 'GET')
    ...
    try {
      ...
      if (list.length > 0) {
        ...
        const { data: feedbackRows } = await supabase
          .from('feedback').select('doctor_id, rating').in('doctor_id', ids);
